...
import { validaUser } from "@/_data/BancoLogin.js"
...
function submitForm() {    
      if (!email.value ) {
        ...
        if (validaUser(email.value, password.value)) {
...
<template>
    <form class="login" @submit.prevent="submitForm">
        ...
        <p>Ainda não possui uma conta? <RouterLink to="/RegisterScreen">Criar conta</RouterLink>
        </p>
    </form>
    <RouterView />
</template>
